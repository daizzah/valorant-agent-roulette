document.addEventListener("DOMContentLoaded", function () {
    ...
    randomizeBtn.addEventListener("click", function () {
      randomizeBtn.textContent = "roll again";
      rollSound.play();
      loading();
    ...
    function loading() {
      hideImages(player1AgentDiv);
      hideImages(player2AgentDiv);
      randomizeGif.style.display = "block"; // <-- SHOW the gif
    ...
    function endLoading() {
      randomizingText.style.display = "none";
    ...
    function displayAgent(div, agent) {
      div.innerHTML = `
      <div style="font-size: 24px; color: #222; margin: 0;">${agent}</div>
      <img src="${agentImages[agent.toLowerCase()]}" alt="${agent}" class="agent-image">
    `;
    }
